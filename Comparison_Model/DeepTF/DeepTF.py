...
class DeepTF(tf.keras.Model):
    def __init__(self, kernal_channel, fc_num,**kwargs):
        super(DeepTF, self).__init__(**kwargs)
        self.kernal_channel=kernal_channel
        # self.fc_num=fc_num
        # self.conv1=layers.Conv2D(filters=self.kernal_channel,kernel_size=7, strides=[1, 1],padding="SAME",name="DeepTF/conv1")
        # self.elu=layers.ELU()
        # self.maxPooling2d=layers.MaxPooling2D(pool_size=[1, 3], strides=[1, 2])
        self.DeepTFBlock1_3 = make_layer(kernal_channel=128, kernel_size=[3, 3], name="DeepTF/DeepTFBlock1_3")
        self.DeepTFBlock1_5 = make_layer(kernal_channel=128, kernel_size=[5, 5], name="DeepTF/DeepTFBlock1_5")
        self.DeepTFBlock1_9 = make_layer(kernal_channel=128, kernel_size=[9, 9], name="DeepTF/DeepTFBlock1_9")

        self.DeepTFBlock2_3 = make_layer(kernal_channel=64, kernel_size=[3, 3], name="DeepTF/DeepTFBlock2_3")
        self.DeepTFBlock2_5 = make_layer(kernal_channel=64, kernel_size=[5, 5], name="DeepTF/DeepTFBlock2_5")
        self.DeepTFBlock2_9 = make_layer(kernal_channel=64, kernel_size=[9, 9], name="DeepTF/DeepTFBlock2_9")

        self.DeepTFBlock3_3 = make_layer(kernal_channel=32, kernel_size=[3, 3], name="DeepTF/DeepTFBlock3_3")
        self.DeepTFBlock3_5 = make_layer(kernal_channel=32, kernel_size=[5, 5], name="DeepTF/DeepTFBlock3_5")
        self.DeepTFBlock3_9 = make_layer(kernal_channel=32, kernel_size=[9, 9], name="DeepTF/DeepTFBlock3_9")

        self.add=layers.Add()

        self.LSTM=layers.LSTM(units=128,activation='tanh')
        # self.averagePooling=layers.AveragePooling2D(pool_size=[ 1, 7], strides=[1, 1])
        self.flatten=layers.Flatten()
        self.dropout=layers.Dropout(rate=0.5)

        # self.dense1=layers.Dense(units=self.fc_num, activation=None,name="DeepTF/dense1")
        self.dense_out = layers.Dense(units=2, activation=None,name="DeepTF/dense_out")
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        x_1_3=self.DeepTFBlock1_3(inputs)
        x_1_5 = self.DeepTFBlock1_5(inputs)
        x_1_9 = self.DeepTFBlock1_9(inputs)

        x_1=self.add([x_1_3,x_1_5])
        x_1 = self.add([x_1, x_1_9])

        x_2_3 = self.DeepTFBlock2_3(x_1)
        x_2_5 = self.DeepTFBlock2_5(x_1)
        x_2_9 = self.DeepTFBlock2_9(x_1)

        x_2 = self.add([x_2_3, x_2_5])
        x_2 = self.add([x_2, x_2_9])

        x_3_3 = self.DeepTFBlock3_3(x_2)
        x_3_5 = self.DeepTFBlock3_5(x_2)
        x_3_9 = self.DeepTFBlock3_9(x_2)

        x_3 = self.add([x_3_3, x_3_5])
        x_3 = self.add([x_3, x_3_9])

        x_3=layers.Reshape((x_3.shape[2],x_3.shape[3]))(x_3)
        x_lstm=self.LSTM(x_3)

        x=self.flatten(x_lstm)
        x=self.dropout(x)
        x=self.dense_out(x)
...
def make_layer( kernal_channel, kernel_size,name):
    # 实现表1中的一个大的残差块
    layers_list = []
    layers_list.append(
        layers.Conv2D(filters=kernal_channel,kernel_size=kernel_size, strides=[1, 1],padding="SAME",name="unit_conv_block1"))
    layers_list.append(
        layers.ReLU())
    layers_list.append(
...
class DeepTFTrainer():
    ...
    def loadModel(self,ckpt_path, kernal_channel=64, fc_num=32):
        model = DeepTF(kernal_channel=kernal_channel, fc_num=fc_num)
    ...
    def train_save(self,epochs,ckpt_save_path):
        ...
        xlsSave_path = os.path.join(ckpt_save_path,"DeepTF.xls")
        # print(xlsSave_path)
        num=0
        for epoch in range(1, epochs + 1):
            ...
            if self.test_loss.result() < best_test_loss:
                ...
                # 保存模型参数
                print(os.path.join(ckpt_save_path,"DeepTF.ckpt"))
                self.model.save_weights(os.path.join(ckpt_save_path, "DeepTF.ckpt"), save_format="tf")
...
def evaluate():
    ...
    for file in os.listdir(subData_path):
        ...
        now_ckpt = os.path.join(subCkpt_path, file, "DeepTF.ckpt")
...
if __name__=='__main__':
    ...
    for file in os.listdir(trainData_path):
        ...
        # xls_path = os.path.join(now_ckpt_save_path, "DeepTF.xls")
        model =DeepTF(kernal_channel=64, fc_num=32)
        trainer = DeepTFTrainer(model=model, data_path=now_trainData_Path, batch_size=128, learning_rate=0.00001)#128非常nice
        trainer.train_save(epochs=20, ckpt_save_path=now_ckpt_save_path)
